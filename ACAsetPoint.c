		...
		if (((ui16_aca_flags & TQ_SENSOR_MODE) != TQ_SENSOR_MODE)) {
			...
			if (ui16_time_ticks_between_pas_interrupt_smoothed > ui16_s_ramp_end) {
				...
				controll_state_temp += 2;
		             ...
		             if(PAS_is_active)
		             uint32_current_target = uint32_temp >>8 +  ui16_current_cal_b; //right shift 15 fasst die Operationen /100 (ann‰hernd >>7) aus der assist_percent und /255 ( >>8) aus dem battery_current max zusammen, ist nicht ganz korrekt, ggf. nur >>14 nehmen -->/(256*128) vs. /(256*64)
